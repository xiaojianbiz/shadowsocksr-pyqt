...
class TCPRelayHandler(object):
    def __init__(self, server, fd_to_handlers, loop, local_sock, config, dns_resolver, is_local):
        ...
        server_ip = self._chosen_server[0]
        server_port = self._chosen_server[1]
        password = self._chosen_server[2]
        method = self._chosen_server[3]
        protocol = self._chosen_server[4]
        protocolparam = self._chosen_server[5]
        obfs_kind = self._chosen_server[6]
        obfsparam = self._chosen_server[7]
        server.protocol_data = obfs.obfs(protocol).init_data()
        server.obfs_data = obfs.obfs(obfs_kind).init_data()
        if common.to_str(protocol) in obfs.mu_protocol():
            server._update_users(None, None)
        self._server = server
        ...
        self._add_ref = 0
        if not self._create_encryptor(method,password):
            return

        self._client_address = local_sock.getpeername()[:2]
        self._accept_address = local_sock.getsockname()[:2]
        self._user = None
        self._user_id = server_port
        ...
        self._obfs = obfs.obfs(obfs_kind)
        self._protocol = obfs.obfs(protocol)
        ...
        server_info.protocol_param = protocolparam
        server_info.obfs_param = obfsparam
        server_info.iv = self._encryptor.cipher_iv
        server_info.recv_iv = b''
        server_info.key_str = common.to_bytes(password)
        ...
        server_info.client = self._client_address[0]
        server_info.client_port = self._client_address[1]
        server_info.protocol_param = protocol
        server_info.obfs_param = protocolparam
        server_info.iv = self._encryptor.cipher_iv
        server_info.recv_iv = b''
        server_info.key_str = common.to_bytes(password)
        server_info.key = self._encryptor.cipher_key
    ...
    def get_server_config(self, object):
        server = object['server']
        server_port = object['server_port']
        password = object['password']
        method = object['method']
        protocol = object['protocol']
        protocolparam = object['protocolparam']
        obfs = object['obfs']
        obfsparam = object['obfsparam']
        logging.debug('chosen server: %s:%d', server, server_port)
        return server, server_port, password, method ,protocol, protocolparam, obfs, obfsparam

    def _get_a_server(self):
        configs = self._config.get('configs',None)
        if not configs:
            ...
            server = random.choice(configs)
            i=1
            while i<100 and server['enable'] == False:
                server = random.choice(configs)
            if server['enable'] == False:
                logging.error('None Server IP')
                return
            else:
                return self.get_server_config(object=server)
    ...
    def _create_encryptor(self, method,password):
        try:
